...
const editProperty = async (values, propertyID) => {
  try {
    const res = await axios.patch(
      `http://127.0.0.1:5000/edit-property/` + propertyID,
      values
    );
  } catch (error) {
    console.log(error);
...
export default function EditPropertyForm(property) {
  const initialFValues = {
    ...
    owner1_id: property.property.owner1_id,
    owner2_id: property.property.owner2_id,
    liability_id: property.property.liablity_id,
  };
  ...
  const handleSubmit = async (e) => {
    if (!validate()) {
      ...
      e.preventDefault();
      editProperty(values, property.property.id);
